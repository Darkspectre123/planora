      ...
      <Stack gap="xs">
        ...
        {total > 0 && (
          <>
            ...
            <Stack gap="xs" mt="xs">
  {sortedSubtasks.slice(0, MAX_VISIBLE_SUBTASKS).map((subtask) => (
    <Group key={subtask._id} direction="column" spacing={2}>
      <Group gap="xs">
        <Checkbox
          checked={subtask.isCompleted}
          onChange={(e) => {
      ...
      {/* Display Start/End dates and priority */}
      <Group spacing="xs">
        {subtask.startDate && (
          <Text size="xs" c="dimmed">
        ...
        {subtask.endDate && (
          <Text size="xs" c="dimmed">
            End: {formatDate(subtask.endDate)}
          </Text>
        )}
        {subtask.priority && (
          <Badge
