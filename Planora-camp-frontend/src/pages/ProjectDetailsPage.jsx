...
const PRIORITY_WEIGHTS = {
  high: { todo: 0, in_progress: 0.3, done: 1 },
  medium: { todo: 0, in_progress: 0.5, done: 1 },
  low: { todo: 0, in_progress: 0.7, done: 1 },
};
...
const weights = currentProject
  ? PRIORITY_WEIGHTS[currentProject.priority?.toLowerCase()]
  : undefined;

const progress = currentProject
  ? getProjectProgress(currentProject._id, weights)
  : 0;

const displayProgress =
  progress === 0 && tasks.length > 0 ? 5 : progress;
  ...
  const taskForm = useForm({
    ...
    validate: {
      ...
      values.startDate && value && value < values.startDate
  ...
  return (
    <Container size="lg" py="xl">
      ...
      <PageHeader
        ...
        title={String(currentProject?.name || "Project").toUpperCase()}
  ...
  <Group position="apart">
    ...
    {currentProject?.endDate &&
      new Date(currentProject.endDate) < new Date() && (
