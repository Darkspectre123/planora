...
const useTaskStore = create((set, get) => ({
  ...
  createSubtask: async (projectId, taskId, subtaskData) => {
    set({ isLoading: true, error: null });
    try {
    const response = await taskService.subtasks.create(
  projectId,
  taskId,
  subtaskData
);
  ...
  updateSubtask: async (projectId, subTaskId, updates) => {
    set({ isLoading: true, error: null });
    try {
      const response = await taskService.subtasks.update(
  projectId,
  subTaskId,
  updates
);
  ...
  toggleSubtaskCompletion: async (projectId, subTaskId, currentStatus) => {
    set({ isLoading: true, error: null });
    try {
      const response = await taskService.subtasks.update(projectId, subTaskId, {
  ...
  deleteSubtask: async (projectId, subTaskId) => {
    set({ isLoading: true, error: null });
    try {
      await taskService.subtasks.delete(projectId, subTaskId);
  ...
  resetStore: () =>
    ...
    getProjectProgress: (projectId, customWeights) => {
  const { tasks } = get();
  ...
  const weights = customWeights || DEFAULT_PROGRESS_WEIGHTS;

  const totalWeight = projectTasks.reduce((sum, task) => {
    return sum + (weights[task.status] ?? 0);
  }, 0);

  return Math.round((totalWeight / projectTasks.length) * 100);
},
